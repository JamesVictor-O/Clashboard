...
function Spotlights({ activeAgent }: { activeAgent: "A" | "B" }) {
  ...
  useFrame(() => {
    if (goldLight.current) {
      goldLight.current.intensity = activeAgent === "A" ? 3 : 0.5;
    }
    if (blueLight.current) {
      blueLight.current.intensity = activeAgent === "B" ? 3 : 0.5;
    }
  });
...
export function ArenaScene({
  ...
  return (
    <div className="relative w-full h-[480px] rounded-xl overflow-hidden border border-white/10">
      {/* Three.js Canvas */}
      <Canvas
        shadows
